...
// Инициализация Supabase клиент для Storage
...
app.post('/api/admin/upload-image',
  ...
  async (req, res) => {
    try {
      ...
      // ✅ ПРАВИЛЬНО: получаем публичный URL
...
app.delete('/api/admin/users/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    // Нельзя удалить другого админа (опционально)
    ...
    console.log(`🗑️ User ${id} deleted by admin ${adminId}`);
...
app.post('/api/admin/products',
  ...
  async (req, res) => {
    try {
      // 1. Добавь img_url в деструктуризацию из req.body
      ...
      // 2. Логика выбора URL:
      let finalImgUrl = img_url || null; // Если пришла строка в JSON, берем её

      if (req.file) {
        // Если всё же прислали файл напрямую
      ...
      const result = await query(
        ...
        [
          ...
          id_category || null,
          id_brand || null,
          finalImgUrl, // Используем итоговый URL
...
app.post('/api/auth/avatar', authMiddleware, async (req, res) => {
  try {
    ...
    // ✅ ЗАГРУЗКА через ADMIN клиент (обходит RLS)
    const { data, error: uploadError } = await supabaseAdmin.storage
      .from('avatars')
    ...
    console.log('🗄️ DB update result:', updateResult.rowCount, 'rows affected');

    if (updateResult.rowCount === 0) {
      console.warn('⚠️ No rows updated - user not found');
      return res.status(404).json({ error: 'Пользователь не найден' });
    }
...
app.put('/api/auth/profile', authMiddleware, async (req, res) => {
  try {
    console.log('✏️ Profile update request:', req.body);
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    // ✅ Поддержка НЕСКОЛЬКИХ категорий (массив или строка)
    if (category) {
    ...
    // ✅ Поддержка НЕСКОЛЬКИХ брендов (массив или строка)
    if (brand) {
...
// ============================================
// CART ROUTES (С ЛОГИРОВАНИЕМ ВРЕМЕНИ)
// ============================================
...
app.post('/api/cart', authMiddleware, async (req, res) => {
  const startTime = Date.now();
  console.log('⏱️ [CART] Request started');
  ...
  try {
    ...
    console.log(`⏱️ [CART] Product check: ${Date.now() - productCheckStart}ms`);
    ...
    console.log(`⏱️ [CART] Existing check: ${Date.now() - existingCheckStart}ms`);
    ...
    if (existingResult.rows.length > 0) {
      ...
      console.log(`⏱️ [CART] Update: ${Date.now() - updateStart}ms`);
      ...
      console.log(`⏱️ [CART] Insert: ${Date.now() - insertStart}ms`);
    ...
    const result = await query(
      `SELECT o.*, 
              json_agg(json_build_object(
                ...
                'img_url', p.img_url  -- ✅ Берём img_url из products
              )) as items
       FROM orders o
       LEFT JOIN order_items oi ON o.id_order = oi.id_order
       LEFT JOIN products p ON oi.id_product = p.id_product  -- ✅ JOIN с products
       WHERE o.id_user = $1
       GROUP BY o.id_order
       ORDER BY o.created_at DESC`,
      [req.user.userId]
...
app.post('/api/admin/clear-pool', async (req, res) => {
  ...
  try {
    ...
    // Пересоздаём пул
    // (нужно импортировать createPool или пересоздать вручную)
...
// Graceful shutdown (ТОЛЬКО ОДИН РАЗ!)
